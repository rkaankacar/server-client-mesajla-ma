# -*- coding: utf-8 -*-
"""
Chat Client - RSA Key Exchange destekli
Protocol entegreli
"""

import socket
import threading
import json
import os

from core.encryption.encryption_factory import EncryptionFactory
from core.network.key_exchange import KeyExchangeClient, is_key_exchange_message
from core.network.protocol import Protocol

class ChatClient:
    def __init__(self, host, port):
        self.host = host
        self.port = port
        self.client_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        
        # Key Exchange
        self.key_exchange = KeyExchangeClient()
        self.symmetric_key = None
        self.symmetric_algo = None
        self.handshake_done = False
        self.preferred_algo = "AES"
        
        # Callbacks
        self.on_key_exchange_complete = None
        self.on_message_received = None
        self.on_file_received = None  # (filename, content)

    def connect(self):
        """Server'a bağlanır."""
        self.client_socket.connect((self.host, self.port))

    def set_preferred_algo(self, algo: str):
        if algo in ["AES", "DES"]:
            self.preferred_algo = algo

    def send(self, message):
        """Raw mesaj gönderir (Protocol wrapper)."""
        Protocol.send_packet(self.client_socket, Protocol.TYPE_TEXT, message)

    def _get_cipher(self, algo_type=None, **kwargs):
        """Cipher nesnesini hazırlar."""
        if self.handshake_done and self.symmetric_key:
            if algo_type is None or algo_type.startswith(self.symmetric_algo):
                if self.symmetric_algo == "AES":
                    from core.encryption.aes_library import AESLibraryCipher
                    cipher = AESLibraryCipher()
                    cipher.key = self.symmetric_key
                    return cipher
                elif self.symmetric_algo == "DES":
                    from core.encryption.des_library import DESLibraryCipher
                    cipher = DESLibraryCipher()
                    cipher.key = self.symmetric_key
                    return cipher
        
        return EncryptionFactory.get_cipher(algo_type, **kwargs)

    def send_encrypted(self, message, algo_type=None, **kwargs):
        """Mesajı şifreler ve gönderir."""
        try:
            cipher = self._get_cipher(algo_type, **kwargs)
            encrypted_message = cipher.encrypt(message)
            print(f"[Client] Text şifrelendi: {encrypted_message[:20]}...")
            
            # Şifreli mesajı normal text paketi olarak gönder
            # (Alan taraf şifreli olduğunu bilecek çünkü normal akış bu)
            Protocol.send_packet(self.client_socket, Protocol.TYPE_TEXT, encrypted_message)
            
        except Exception as e:
            print(f"Şifreleme hatası: {e}")

    def send_file_encrypted(self, filepath, algo_type=None, **kwargs):
        """Dosyayı şifreler ve gönderir."""
        try:
            if not os.path.exists(filepath):
                print("Dosya bulunamadı")
                return

            filename = os.path.basename(filepath)
            
            # 1. Dosyayı oku
            with open(filepath, 'rb') as f:
                file_data = f.read()

            # 2. Şifrele (Bytes olarak)
            cipher = self._get_cipher(algo_type, **kwargs)
            
            # Not: Bazı cipher'lar string bekleyebilir, bytes'a çevirmeliyiz
            # EncryptionFactory'deki cipherların çoğu string çalışıyor gibi görünüyor,
            # ama AESLibraryCipher bytes desteklemeli.
            # Eğer text tabanlı bir cipher seçildiyse (Sezar vb), binary dosya bozulabilir.
            # Bu yüzden dosya gönderiminde SADECE modern algoritmalar önerilir.
            
            if hasattr(cipher, 'encrypt_bytes'):
                 encrypted_data = cipher.encrypt_bytes(file_data)
            else:
                # Fallback: Eğer library cipher değilse, bytes->str dönüşümü (riskli)
                # Deneme amaçlı: latin-1 decode/encode
                try:
                    str_data = file_data.decode('latin-1')
                    enc_str = cipher.encrypt(str_data)
                    encrypted_data = enc_str.encode('latin-1')
                except:
                     # Cipher encrypt metodu bytes kabul ediyorsa
                     encrypted_data = cipher.encrypt(file_data)

            # 3. Metadata Gönder
            metadata = {
                "filename": filename,
                "size": len(encrypted_data),
                "original_size": len(file_data),
                "encrypted": True
            }
            Protocol.send_packet(self.client_socket, Protocol.TYPE_FILE_METADATA, json.dumps(metadata))

            # 4. İçeriği Gönder
            Protocol.send_packet(self.client_socket, Protocol.TYPE_FILE_CHUNK, encrypted_data)
            
            print(f"[Client] Dosya gönderildi: {filename}")

        except Exception as e:
            print(f"Dosya gönderme hatası: {e}")

    def start_receiving(self, handler, file_handler=None):
        self.on_message_received = handler
        self.on_file_received = file_handler
        threading.Thread(target=self._receive, daemon=True).start()

    def _receive(self):
        current_file_metadata = None

        while True:
            try:
                packet_type, payload = Protocol.recv_packet(self.client_socket)
                if packet_type is None:
                    break
                
                if packet_type == Protocol.TYPE_TEXT or packet_type == Protocol.TYPE_KEY_EXCHANGE:
                    # Text verisi (genellikle string decode edilir)
                    try:
                        data = payload.decode('utf-8')
                    except:
                        data = payload.decode('latin-1')
                        
                    if is_key_exchange_message(data):
                        self._process_key_exchange(data)
                    else:
                        if self.on_message_received:
                            self.on_message_received(data)

                elif packet_type == Protocol.TYPE_FILE_METADATA:
                    try:
                        meta_json = payload.decode('utf-8')
                        current_file_metadata = json.loads(meta_json)
                        print(f"[Client] Dosya geliyor: {current_file_metadata['filename']}")
                    except Exception as e:
                        print(f"Metadata hatası: {e}")
                
                elif packet_type == Protocol.TYPE_FILE_CHUNK:
                    # Dosya içeriği (şifreli)
                    if current_file_metadata and self.on_file_received:
                        self.on_file_received(current_file_metadata, payload)
                        current_file_metadata = None # Reset
            
            except Exception as e:
                print(f"[Client] Alma hatası: {e}")
                break

    def _process_key_exchange(self, data):
        """Server'dan gelen key exchange mesajlarını işler."""
        try:
            parsed = json.loads(data)
            msg_type = parsed.get("type", "")
            
            if msg_type == "KEY_EXCHANGE_INIT":
                print("[Client] Server public key alındı")
                
                if self.key_exchange.process_server_handshake(data):
                    self.key_exchange.generate_symmetric_key(self.preferred_algo)
                    self.symmetric_key = self.key_exchange.get_symmetric_key()
                    self.symmetric_algo = self.key_exchange.get_symmetric_algo()
                    
                    print(f"[Client] {self.symmetric_algo} anahtarı oluşturuldu: {self.symmetric_key.hex()}")
                    
                    response = self.key_exchange.create_key_response()
                    # Key Exchange mesajları normal text olarak gidiyor (JSON)
                    Protocol.send_packet(self.client_socket, Protocol.TYPE_KEY_EXCHANGE, response)
                    print("[Client] Şifreli anahtar gönderildi")
            
            elif msg_type == "KEY_EXCHANGE_ACK":
                if parsed.get("status") == "success":
                    self.handshake_done = True
                    print(f"[Client] ✅ Key Exchange tamamlandı! Algoritma: {self.symmetric_algo}")
                    
                    if self.on_key_exchange_complete:
                        self.on_key_exchange_complete(self.symmetric_algo, self.symmetric_key)
                        
        except Exception as e:
            print(f"[Client] Key exchange hatası: {e}")

    def is_handshake_done(self) -> bool:
        return self.handshake_done

    def get_symmetric_key(self) -> bytes:
        return self.symmetric_key

    def get_symmetric_algo(self) -> str:
        return self.symmetric_algo
